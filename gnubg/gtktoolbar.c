...
ToolbarNew(void)
{
    ...
    ptw->pwAnalyzeCurrent =
        ToolbarAddButton(GTK_TOOLBAR(pwtb), GTK_STOCK_UNDO, NULL, _("Analyze current match (set default behavior in Settings -> Analysis -> Analysis Buttons)"), G_CALLBACK(GTKAnalyzeCurrent), NULL);

    /* Analyze file button */
    ptw->pwAnalyzeFile =
        ToolbarAddButton(GTK_TOOLBAR(pwtb), GTK_STOCK_UNDO, NULL, _("Analyze match from file (set default behavior in Settings -> Analysis -> Analysis Buttons)"), G_CALLBACK(GTKAnalyzeFile), NULL);
